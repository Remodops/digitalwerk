...
export default function Page() {
  ...
  return (
    <>
      ...
      <Section title="Leistungsbereiche" subtitle="Was wir konkret für dein Projekt umsetzen" className="bg-neutral-50 py-6 sm:py-8 dark:bg-neutral-950">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {services.map((s, i) => {
            const iconBg = ["bg-emerald-50", "bg-sky-50", "bg-violet-50", "bg-amber-50"][i % 4];
            const cardAltBg = i % 2 === 1 ? "bg-neutral-50" : "";
            return (
              <Card key={s.title} className={`h-full p-5 ${cardAltBg} dark:bg-neutral-900`}>
                <div className="flex items-center gap-3">
                  <span aria-hidden className={`flex h-8 w-8 items-center justify-center rounded-full ${iconBg} dark:bg-neutral-800`}>{s.icon}</span>
                  <h3 className="text-lg font-semibold text-neutral-900">{s.title}</h3>
                </div>
                <p className="mt-3 text-sm text-neutral-700 dark:text-neutral-300">{s.description}</p>
                <div className="mt-3">
      ...
      <Section id="preise" title="Pakete" subtitle="Drei klare Pakete – flexibel erweiterbar." className="py-6 sm:py-8">
        <p className="text-sm text-neutral-700 dark:text-neutral-300 mb-3">Transparente Preise – flexibel erweiterbar, ohne versteckte Kosten.</p>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {packages.map((p) => (
            <div key={p.name} className="rounded-xl border bg-neutral-50 p-5 shadow-sm flex h-full flex-col dark:bg-neutral-900 dark:border-neutral-800">
              <div className="flex items-center gap-2">
                <span aria-hidden className="text-xl">{p.icon}</span>
                <h3 className="text-lg font-semibold">{p.name}</h3>
              </div>
              <p className="mt-1 text-neutral-600 dark:text-neutral-300">{p.price}</p>
      ...
      <Section title="Optionale Pakete" className="py-6 sm:py-8">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {carePackages.map((p) => (
            <div key={p.name} className="rounded-xl border bg-white p-5 shadow-sm dark:bg-neutral-900 dark:border-neutral-800">
              <h3 className="text-lg font-semibold">{p.name}</h3>
              <p className="mt-1 text-neutral-600 dark:text-neutral-300">{p.priceMonthly}</p>
              ...
              return (
                <li key={i} className="flex flex-col items-center text-center">
                  ...
                  <div className="relative mt-1 w-full h-12">
                    {i > 0 && (
                      <div className="absolute left-0 right-[calc(50%+26px)] top-1/2 -translate-y-1/2 hidden h-px bg-neutral-200 dark:bg-neutral-800 z-0 sm:block" aria-hidden />
                    )}
                    {i < arr.length - 1 && (
                      <div className="absolute left-[calc(50%+26px)] right-0 top-1/2 -translate-y-1/2 hidden h-px bg-neutral-200 dark:bg-neutral-800 z-0 sm:block" aria-hidden />
                    )}
                  ...
                  <div className="mt-3 font-semibold tracking-wide dark:text-white">{step.key}</div>
                  <p className="mt-1 text-sm text-neutral-700 dark:text-neutral-300 max-w-[30ch] mx-auto sm:mx-0 leading-relaxed tracking-[0.005em]">{desc[step.key as keyof typeof desc]}</p>
                  {step.key === "Launch" && (
                    <div className="mt-2">
